...
main :: proc() {
    ...
    hud := Hud{false, 0, WORKBENCH_BLUE, rl.LoadRenderTexture(WIDTH, HUD_HEIGHT)}
    pixels_per_frame := 1
    ...
    for !rl.WindowShouldClose() {
        ...
        if !paused {
            for i in 0..<pixels_per_frame {
                ...
                rl.DrawPixel(x, y, {u8(r), u8(g), u8(b), 255})
            }
        }
        rl.EndTextureMode()

        rl.BeginTextureMode(hud.target)
        rl.ClearBackground(WORKBENCH_BLUE)
        text := rl.TextFormat("Pixels per frame: %03d", pixels_per_frame)
        rl.DrawText(text, 10, 10, FONT_SIZE, rl.RAYWHITE)

        if paused && pause_blink_counter < BLINK_CYCLE / 2 {
            x := WIDTH / 2 - rl.MeasureText("PAUSED", FONT_SIZE) / 2
            rl.DrawText("PAUSED", x, 10, FONT_SIZE, rl.RAYWHITE)
        }

        rl.DrawRectangle(WIDTH - 66, 10, 56, 20, rl.RAYWHITE)
        if use_red do rl.DrawRectangle(WIDTH - 64, 12, 16, 16, rl.RED)
        if use_green do rl.DrawRectangle(WIDTH - 46, 12, 16, 16, rl.GREEN)
        if use_blue do rl.DrawRectangle(WIDTH - 28, 12, 16, 16, rl.BLUE)
        rl.EndTextureMode()

        rl.BeginDrawing()
        rl.DrawTextureRec(target.texture, {0, 0, WIDTH, -HEIGHT}, {0, 0}, rl.WHITE)
        if hud.draw || hud.current_height > 0 {
            src_rect := rl.Rectangle{0, HUD_HEIGHT, WIDTH, -f32(hud.current_height)}
            dest_rect := rl.Rectangle{0, 0, WIDTH, f32(hud.current_height)}
            rl.DrawTexturePro(hud.target.texture, src_rect, dest_rect, {0, 0}, 0.0, rl.RAYWHITE )
        }

        rl.EndDrawing()
    }

    rl.UnloadRenderTexture(hud.target)
