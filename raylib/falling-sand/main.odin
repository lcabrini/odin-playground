...
main :: proc() {
	...
	for !rl.WindowShouldClose() {
		if rl.IsMouseButtonDown(rl.MouseButton.LEFT) {
			...
			for i in 0 ..< 300 {
				l := 100 * rand.float32() - 50
				a := 360 * rand.float32() * rl.DEG2RAD
				x := int(mp.x + l * math.cos(a))
				y := int(mp.y + l * math.sin(a))
				if y < 0 || y > HEIGHT - 1 do continue
				if x < 0 || x > WIDTH - 1 do continue
				grain := Grain{}
				grain.v = {0, f32(rl.GetRandomValue(1, 4))}
				change := u8(rl.GetRandomValue(-20, 20))
				grain.color = {0xdc + change, 0xb1 + change, 0x59 + change, 255}
			...
			for x in 0 ..< WIDTH {
				...
				case Grain:
					if screen[(y + 1) * WIDTH + x] != empty {
						if screen[(y + 1) * WIDTH + x - 1] == empty &&
						   screen[(y + 1) * WIDTH + x + 1] == empty {
						...
						} else if (y + 1) * WIDTH + x + 1 < len(screen) &&
						   screen[(y + 1) * WIDTH + x + 1] == empty {
							if x + 1 < WIDTH - 1 {
								screen[(y + 1) * WIDTH + x + 1] = pt
								screen[y * WIDTH + x] = empty
